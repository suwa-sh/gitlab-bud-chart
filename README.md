...
1. `created_at` が期間終了日より未来の Issue は除外
2. `completed_at` が期間終了日より未来の Issue は除外
3. `created_at` が期間内の Issue
4. `created_at` が期間外でも `state=opened` の Issue
5. `completed_at` が期間内の Issue
...
gantt
    ...
    section 含まれるIssue
    期間内作成完了        :done, rule1_done, 2024-01-15, 2024-02-15
    期間内作成進行中      :active, rule1_active, 2024-02-01, 2024-04-30
    期間外作成Opened     :active, rule2, 2023-11-15, 2024-06-01
    期間外作成期間内完了  :done, rule3, 2023-12-01, 2024-02-01
    日付補正後通過       :done, corrected, 2024-01-20, 2024-01-20

    section 除外されるIssue
    未来作成Issue       :crit, rule4_future, 2024-04-15, 2024-05-15
    未来完了Issue       :crit, rule5_future, 2024-02-01, 2024-04-15
    ...
    今日               :milestone, today, 2024-03-22, 0d
```
...
| ----- | ---------------- | ------------- | ---------------- | ------------ | ------ | -------- | ------------------------------------ |
| A     | 3 段階通過       | #完了         | 2024-01-15       | 2024-02-15   | closed | ✅ 含む  | スコープルール 1 (期間内作成)        |
| B     | 3 段階通過       | #作業中       | 2024-02-01       | -            | opened | ✅ 含む  | スコープルール 1 (期間内作成)        |
| C     | 3 段階通過       | #完了         | 2023-12-01       | 2024-02-01   | closed | ✅ 含む  | スコープルール 3 (期間内完了)        |
| D     | 3 段階通過       | #作業中       | 2023-11-01       | -            | opened | ✅ 含む  | スコープルール 2 (期間外作成+opened) |
| E     | スコープ除外     | #完了         | 2024-02-15       | 2024-04-15   | opened | ❌ 除外  | スコープルール 5 (未来完了)          |
| F     | 日付補正適用     | #完了         | 2024-01-25→01-20 | 2024-01-20   | closed | ✅ 含む  | 補正後スコープルール 1               |
| G     | スコープ除外     | #作業中       | 2024-04-15       | -            | opened | ❌ 除外  | スコープルール 4 (未来作成)          |
...
2. **日付補正**: `created_at > completed_at`の場合、`created_at = completed_at`に修正
3. **スコープ判定**: 5 つのルールで期間内外・未来日付の包含/除外を決定
