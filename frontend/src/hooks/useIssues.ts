  ...
  const fetchIssues = useCallback(
    async (params: any = {}) => {
      ...
      try {
        ...
        if (params.period) {
          const overlappingQuarters = getOverlappingQuarters(
            params.period.start,
            params.period.end,
          )
          ...
          if (overlappingQuarters.length > 0) {
            ...
            if (response.issues) {
              const filteredIssues = response.issues.filter((issue: any) =>
                overlappingQuarters.some(
                  (quarter) =>
                    normalizeQuarterLabel(quarter) ===
                    normalizeQuarterLabel(issue.quarter || ''),
                ),
              )
