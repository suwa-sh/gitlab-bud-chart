...
export const usePerformance = () => {
  ...
  const optimizeSearchResults = useCallback(
    (issues: Issue[], searchTerm: string) => {
  ...
  const optimizeFiltering = useCallback((issues: Issue[], filters: any) => {
    ...
    return issues.filter((issue) => {
      ...
      // ポイント範囲フィルタ
      if (
        filters.min_point !== undefined &&
        (issue.point || 0) < filters.min_point
      ) {
        return false
      }

      if (
        filters.max_point !== undefined &&
        (issue.point || 0) > filters.max_point
      ) {
        return false
      }
  ...
  // ソート最適化
  const optimizeSorting = useCallback(
    (issues: Issue[], sortBy: string, sortOrder: 'asc' | 'desc') => {
  ...
  const optimizeStatistics = useMemo(() => {
    return (issues: Issue[]) => {
      ...
      issues.forEach((issue) => {
        ...
        // サービス別カウント
        if (issue.service) {
          stats.by_service[issue.service] =
            (stats.by_service[issue.service] || 0) + 1
        }

        // 担当者別カウント
        if (issue.assignee) {
          stats.by_assignee[issue.assignee] =
            (stats.by_assignee[issue.assignee] || 0) + 1
        }
      })
