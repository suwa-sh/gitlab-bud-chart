...
export const GitLabConfig = ({
  ...
  const [config, setConfig] = useState({
    gitlab_url:
      editMode && state.gitlabConfig.url
        ? state.gitlabConfig.url
        : getEnv('VITE_GITLAB_URL', 'http://localhost:8080'),
    gitlab_token:
      editMode && state.gitlabConfig.token
        ? state.gitlabConfig.token
        : getEnv('VITE_GITLAB_TOKEN', ''),
    project_id:
      editMode && state.gitlabConfig.projectId
        ? state.gitlabConfig.projectId
        : getEnv('VITE_GITLAB_PROJECT_ID', '1'),
    api_version:
      editMode && state.gitlabConfig.apiVersion
        ? state.gitlabConfig.apiVersion
        : getEnv('VITE_GITLAB_API_VERSION', '4'),
    http_proxy:
      editMode && state.gitlabConfig.httpProxy
        ? state.gitlabConfig.httpProxy
        : getEnv('VITE_HTTP_PROXY', ''),
    https_proxy:
      editMode && state.gitlabConfig.httpsProxy
        ? state.gitlabConfig.httpsProxy
        : getEnv('VITE_HTTPS_PROXY', ''),
    no_proxy:
      editMode && state.gitlabConfig.noProxy
        ? state.gitlabConfig.noProxy
  ...
  useEffect(() => {
    const validateAndFetchProjects = async () => {
      if (
        config.gitlab_url &&
        config.gitlab_token &&
        config.gitlab_url.length > 5 &&
        config.gitlab_token.length > 10
      ) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      setStatus(
        `✓ GitLab接続済み: ${result.project_info.project?.name || 'Unknown'}`,
      )
      ...
      const selectedProject = projects.find(
        (p) => p.id.toString() === config.project_id,
      )
      ...
      let projectNamespace =
        ...
        result.project_info.project?.path_with_namespace
      ...
      dispatch({
        ...
        payload: {
          ...
          projectName:
            ...
            result.project_info.project?.name ||
            config.project_id,
      ...
      <div className="config-form">
                ...
                {!credentialsValid &&
                  ...
                  config.gitlab_token && (
                    <div className="validation-hint">
                      有効なURLとトークンを入力するとプロジェクト一覧が表示されます
                    </div>
                  )}
              </div>
            )}
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) =>
              setConfig((prev) => ({ ...prev, api_version: e.target.value }))
            }
            placeholder="4"
          />
        </div>
            ...
            <div className="advanced-fields">
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setConfig((prev) => ({ ...prev, no_proxy: e.target.value }))
                  }
