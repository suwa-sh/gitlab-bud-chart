 ...
 */
export const generateQuarterOptions = (): Array<{
  value: string
  label: string
}> => {
  ...
  const currentQuarter =
    currentMonth >= 4 ? Math.floor((currentMonth - 4) / 3) + 1 : 4
  ...
  for (let year = currentYear - 2; year <= currentYear + 1; year++) {
    for (let q = 1; q <= 4; q++) {
      ...
      if (
        ...
        (year === currentFiscalYear && q > currentQuarter + 2)
      )
        continue
