...
test.describe("Full Application Workflow", () => {
  test("complete user journey: setup → data fetch → analysis → visualization", async ({
    page,
  }) => {
    // 1. GitLab接続設定
    ...
    await page.screenshot({
      path: "test-results/integration-01-gitlab-connected.png",
    });
    ...
    await page.screenshot({
      path: "test-results/integration-02-issues-loaded.png",
    });
    ...
    await page.screenshot({
      path: "test-results/integration-03-filters-applied.png",
    });
    ...
    await page.screenshot({
      path: "test-results/integration-04-charts-displayed.png",
    });
    ...
    await page.screenshot({
      path: "test-results/integration-05-pbl-viewer.png",
    });
    ...
    await page.screenshot({
      path: "test-results/integration-06-statistics.png",
    });
  });
  ...
  test("error handling and recovery", async ({ page }) => {
    ...
    // 再接続試行可能確認
    await expect(
      page.getByRole("button", { name: "接続", exact: true })
    ).toBeEnabled();
  });
  ...
  test("performance and responsiveness", async ({ page }) => {
    ...
    if (await gitlabConfig.isVisible()) {
      ...
      await page.getByLabel("Access Token").fill(testConfig.access_token);
      await page
        .getByLabel("Project ID")
        .fill(testConfig.project_id.toString());
      await page.getByRole("button", { name: "接続", exact: true }).click();
