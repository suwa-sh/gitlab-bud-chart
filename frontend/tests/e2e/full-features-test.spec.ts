...
test.describe("Full Features Test with 20 Issues", () => {
  test("complete feature test: GitLab connection → Charts → Issue List → PBL Viewer", async ({
    page,
  }) => {
    // コンソールログを監視
    page.on("console", (msg) => {
    ...
    if (isDashboardVisible) {
      ...
      await page.screenshot({
        path: "test-results/full-test-01-dashboard.png",
        fullPage: true,
      });
    } else {
      console.log("  - ❌ Dashboard表示失敗");
      await page.screenshot({
        path: "test-results/full-test-01-error.png",
        fullPage: true,
      });
    }
    ...
    if (await chartSection.isVisible()) {
      ...
      await page.screenshot({
        path: "test-results/full-test-02-charts.png",
        fullPage: true,
      });
    } else {
    ...
    if (await issueSection.isVisible()) {
      ...
      await page.screenshot({
        path: "test-results/full-test-03-issues.png",
        fullPage: true,
      });
    }
    ...
    if (await filterButton.isVisible()) {
      ...
      await page.screenshot({
        path: "test-results/full-test-04-filtered.png",
        fullPage: true,
      });
    ...
    // PBL Viewer表示確認
    const pblHeader = page.getByRole("heading", {
      name: "Product Backlog Viewer",
    });
    if (await pblHeader.isVisible()) {
      ...
      await page.screenshot({
        path: "test-results/full-test-05-pbl-viewer.png",
        fullPage: true,
      });
    }
    ...
    await page.screenshot({
      path: "test-results/full-test-06-final-dashboard.png",
      fullPage: true,
    });
  ...
  test("verify chart data accuracy", async ({ page }) => {
    ...
    if (burnDownResponse.ok()) {
      ...
      console.log("  - Burn-down API Response:", {
        ...
        completedPoints:
          burnDownData.statistics?.initial_points -
          burnDownData.statistics?.final_points,
      });
    }
