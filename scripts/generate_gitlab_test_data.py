...
def create_project_and_issues(
    token, gitlab_url, project_name=None, project_description=None
):
    """プロジェクト作成とテストイシューを作成"""
    if not token:
    ...
    if project_status_code == 201:
        ...
        projects_request = urllib.request.Request(
            f"{gitlab_url}/api/v4/projects", headers=headers
        )
    ...
    labels = [
        ...
        # 統一フィルタ除外対象
        {
            "name": "#テンプレート",
            "color": "#95a5a6",
            "description": "カンバン: テンプレート（除外対象）",
        },
        {
            "name": "#ゴール/アナウンス",
            "color": "#34495e",
            "description": "カンバン: ゴール/アナウンス（除外対象）",
        },
        {
            "name": "#不要",
            "color": "#7f8c8d",
            "description": "カンバン: 不要（除外対象）",
        },
        # サービス分類
        ...
        {"name": "s:frontend", "color": "#7f7f7f", "description": "サービス: frontend"},
        {
            ...
            "color": "#27ae60",
            "description": "サービス: infrastructure",
        },
    ...
    for milestone in milestones:
        ...
        if milestone_status_code == 201:
            ...
            print(
                f"マイルストーン作成失敗: {milestone['title']} - {milestone_status_code}"
            )
    ...
    milestones_request = urllib.request.Request(
        f"{gitlab_url}/api/v4/projects/{project_id}/milestones", headers=headers
    )
    ...
    if milestones_status_code == 200:
        ...
        q1_milestone = next(
            (m for m in milestones_data if m["title"] == "2024Q1"), None
        )
...
def main():
    ...
    try:
        ...
        # プロジェクトとイシュー作成
        project_id, access_token = create_project_and_issues(
            token, gitlab_url, project_name, project_description
        )
        ...
        print(
            "2. .envファイルにVITE_GITLAB_TOKENとVITE_GITLAB_URLが正しく設定されていることを確認してください"
        )
