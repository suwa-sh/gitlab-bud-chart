...
async def connect_gitlab(
    config: GitLabConfigRequest, x_session_id: Optional[str] = Header(None)
):
    """GitLab接続設定"""
    # セッションIDがない場合は新規作成
    if not x_session_id:
    ...
    if success:
        ...
        raise HTTPException(status_code=400, detail="GitLab接続に失敗しました")


@router.get("/status")
@router.get("/status/")
async def get_gitlab_status(x_session_id: Optional[str] = Header(None)):
    """GitLab接続状態確認"""
    if not x_session_id:
...
async def get_sample_issues(x_session_id: Optional[str] = Header(None)):
    """サンプルissue取得（動作確認用）"""
    if not x_session_id:
        raise HTTPException(status_code=401, detail="セッションIDが必要です")
...
async def validate_gitlab_credentials(
    config: GitLabValidateRequest, x_session_id: Optional[str] = Header(None)
):
