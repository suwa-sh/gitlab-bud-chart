...
def _apply_scope_filter(
    ...
    for issue in issues:
        ...
        if issue.completed_at:
            ...
            completed_date = (
                issue.completed_at.astimezone(timezone.utc).date()
                if issue.completed_at.tzinfo
                else issue.completed_at.date()
            )
...
def _sort_issues(
    issues: List[IssueModel], sort_by: str, sort_order: str
) -> List[IssueModel]:
    """Issue ソート"""
    ...
    if sort_by == "created_at":
        ...
        return sorted(
            issues, key=lambda x: x.updated_at or x.created_at, reverse=reverse
        )
    elif sort_by == "point":
...
def _paginate_issues(
    issues: List[IssueModel], page: int, per_page: int
) -> List[IssueModel]:
    """ページネーション"""
...
def _collect_metadata(issues: List[IssueModel]) -> Dict[str, Any]:
    ...
    return {
        ...
        "kanban_statuses": sorted(
            list(set(i.kanban_status for i in issues if i.kanban_status))
        ),
    }
...
async def get_issues(
    ...
    try:
        ...
        normalized_kanban_status = (
            kanban_status if kanban_status and kanban_status.strip() else None
        )
        ...
        if chart_start_date and chart_end_date:
            issues, warnings = apply_scope_filters(
                issues, chart_start_date, chart_end_date
            )
        else:
        ...
        for warning in warnings:
            excluded_issues.append(
                ExcludedIssue(
                    issue=_issue_to_response(warning["issue"]), reason=warning["reason"]
                )
            )
...
async def get_analyzed_issues(
    ...
    try:
        ...
        response = {"issues": issue_responses, "total_count": len(issue_responses)}
        ...
        raise HTTPException(
            status_code=500, detail=f"分析済みIssues取得に失敗しました: {str(e)}"
        )


@router.get("/validation", response_model=Dict[str, Any])
async def validate_issues_data(x_session_id: Optional[str] = Header(None)):
    """Issue分析データ検証"""
    if not x_session_id:
        raise HTTPException(status_code=401, detail="セッションIDが必要です")
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "validation_rate": round(
                    (total_issues - issues_with_errors) / total_issues, 4
                )
                if total_issues > 0
                else 0,
            },
        ...
        raise HTTPException(
            status_code=500, detail=f"Issue検証に失敗しました: {str(e)}"
        )
...
async def get_issues_statistics(
    ...
    try:
        ...
        issues, base_statistics = await issue_service.get_analyzed_issues(
            milestone=milestone, labels=labels if labels else None, analyze=True
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Issue統計取得に失敗しました: {str(e)}"
        )
...
async def search_issues(
    ...
    try:
        ...
        filtered_issues = _apply_advanced_filters(
            issues,
            search_request.min_point,
            search_request.max_point,
            search_request.query,
            search_request.kanban_status,
            search_request.is_epic,
        )
        ...
        return {
            ...
            "total_pages": (len(filtered_issues) + search_request.per_page - 1)
...
async def export_issues_csv(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"CSV エクスポートに失敗しました: {str(e)}"
        )


@router.get("/milestone/{milestone_name}", response_model=IssueListResponse)
async def get_issues_by_milestone(
    milestone_name: str, x_session_id: Optional[str] = Header(None)
):
    """マイルストーン別issue取得"""
    if not x_session_id:
        raise HTTPException(status_code=401, detail="セッションIDが必要です")
...
async def get_issue(issue_id: int, x_session_id: Optional[str] = Header(None)):
    """特定issue詳細取得"""
    if not x_session_id:
        raise HTTPException(status_code=401, detail="セッションIDが必要です")
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Issue詳細取得に失敗しました: {str(e)}"
        )
