...
async def get_burn_down_data(
    ...
    if start_date >= end_date:
        raise HTTPException(
            status_code=400, detail="終了日は開始日より後の日付を指定してください"
        )
            ...
            formatted_warnings.append(
                {
                    "issue": {
                        ...
                        "due_date": warning["issue"].due_date.isoformat()
                        if warning["issue"].due_date
                        else None,
                        "completed_at": warning["issue"].completed_at.isoformat()
                        if warning["issue"].completed_at
                        else None,
                        "created_at": warning["issue"].created_at.isoformat()
                        if warning["issue"].created_at
                        else None,
                    },
...
async def get_burn_up_data(
    ...
    try:
        ...
        # チャートデータ生成（事前フィルタリング済みのissuesを渡す）
        chart_data = chart_analyzer.generate_burn_up_data(issues, start_date, end_date)
...
async def get_velocity_data(
    weeks: int = Query(12, ge=1, le=52), x_session_id: Optional[str] = Header(None)
):
    """ベロシティデータ取得"""
    if not x_session_id:
        raise HTTPException(status_code=401, detail="セッションIDが必要です")
    ...
    try:
        ...
        return {
            ...
            "average_velocity": sum(v["completed_points"] for v in velocity_data)
            / len(velocity_data)
...
def _calculate_chart_statistics(
    chart_data: List[ChartDataModel], chart_type: str
) -> dict:
    """チャート統計計算"""
    if not chart_data:
        return {}
    ...
    if chart_type == "burn_down":
        return {
            "completion_rate": (initial_data.actual_points - final_data.actual_points)
            / initial_data.actual_points
            if initial_data.actual_points > 0
            else 0,
        ...
        return {
            "completion_rate": final_data.completed_points / final_data.total_points
            if final_data.total_points > 0
            else 0,
