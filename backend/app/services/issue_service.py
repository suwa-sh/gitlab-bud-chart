...
class IssueService:
    ...
    async def get_issues_by_milestone(
        self, milestone: str, state: str = "all"
    ) -> List[IssueModel]:
        """マイルストーン別issue取得"""
        return await self.get_all_issues(state=state, milestone=milestone)
    ...
    def _convert_to_model(self, gitlab_issue) -> IssueModel:
        ...
        try:
            return IssueModel(
                ...
                due_date=self._parse_datetime(gitlab_issue.due_date)
                if gitlab_issue.due_date
                else None,
                assignee=gitlab_issue.assignee["name"]
    ...
    def _parse_datetime(self, date_str: Optional[str]) -> Optional[datetime]:
        ...
        try:
            ...
            logger.debug(
                f"日時解析成功: {date_str} -> {parsed_dt} (tzinfo: {parsed_dt.tzinfo})"
            )
            return parsed_dt
    ...
    async def get_analyzed_issues(
        ...
        issues = await self.get_all_issues(
            state=state, milestone=milestone, assignee=assignee, labels=labels
        )
    ...
    def _generate_statistics(self, issues: List[IssueModel]) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_points = (
                total_points / len(point_issues) if len(point_issues) > 0 else 0
            )
            ...
            for issue in issues:
                if issue.kanban_status:
                    kanban_counts[issue.kanban_status] = (
                        kanban_counts.get(issue.kanban_status, 0) + 1
                    )
            ...
            for issue in issues:
                if issue.service:
                    service_counts[issue.service] = (
                        service_counts.get(issue.service, 0) + 1
                    )

            # 完了率
            completed_issues = [
                i for i in issues if i.completed_at or i.state == "closed"
            ]
            completion_rate = (
                len(completed_issues) / total_count if total_count > 0 else 0
            )
