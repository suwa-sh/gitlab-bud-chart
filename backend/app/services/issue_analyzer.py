...
class IssueAnalyzer:
    ...
    def analyze_issue(self, issue: IssueModel) -> IssueModel:
        ...
        try:
            ...
            logger.debug(
                f"Issue分析完了 (ID: {issue.id}): point={issue.point}, "
    ...
    def _log_analysis_statistics(self, issues: List[IssueModel]) -> None:
        ...
        logger.info(
            f"  Point設定済み: {point_count}/{total} ({point_count / total * 100:.1f}%)"
        )
        logger.info(
            f"  Kanban Status設定済み: {kanban_count}/{total} ({kanban_count / total * 100:.1f}%)"
        )
        logger.info(
            f"  Service設定済み: {service_count}/{total} ({service_count / total * 100:.1f}%)"
        )
        logger.info(
            f"  Quarter設定済み: {quarter_count}/{total} ({quarter_count / total * 100:.1f}%)"
        )
        logger.info(
            f"  完了日設定済み: {completed_count}/{total} ({completed_count / total * 100:.1f}%)"
        )
    ...
    def get_unique_values(self, issues: List[IssueModel]) -> Dict[str, List[str]]:
        ...
        try:
            ...
            unique_services = list(
                set(issue.service for issue in issues if issue.service is not None)
            )

            unique_quarters = list(
                set(issue.quarter for issue in issues if issue.quarter is not None)
            )

            unique_points = sorted(
                list(set(issue.point for issue in issues if issue.point is not None))
            )
            ...
            return {"kanban_statuses": [], "services": [], "quarters": [], "points": []}
