...
class ChartAnalyzer:
    ...
    def generate_burn_down_data(
        self, issues: List[IssueModel], start_date: date, end_date: date
    ) -> List[ChartDataModel]:
        """Burn-downチャートデータ生成
    ...
    def _filter_by_milestone(
        self, issues: List[IssueModel], milestone: Optional[str]
    ) -> List[IssueModel]:
        """マイルストーンフィルタ"""
        if not milestone:
            return issues
    ...
    def _calculate_completed_points_by_date(
        self, issues: List[IssueModel], target_date: date
    ) -> float:
        """指定日時点での完了ポイント計算"""
        completed_points = 0
        for issue in issues:
    ...
    def _calculate_ideal_remaining(
        self, total_points: float, start_date: date, end_date: date, current_date: date
    ) -> float:
    ...
    def _validate_burn_down_data(
        self, chart_data: List[ChartDataModel], total_points: float
    ) -> None:
    ...
    def _calculate_total_points_by_date(
        self, issues: List[IssueModel], date_range: List[date]
    ) -> Dict[date, float]:
        """日付別総ポイント計算（スコープ変更対応）"""
        total_by_date = {}
        ...
        for target_date in date_range:
            ...
            for issue in issues:
                ...
                if issue.created_at and issue.point:
                    ...
                    created_date = (
                        issue.created_at.astimezone(timezone.utc).date()
                        if issue.created_at.tzinfo
                        else issue.created_at.date()
                    )
