...
class DataQualityChecker:
    ...
    def _check_data_completeness(self, issues: List[IssueModel]) -> Dict[str, float]:
        ...
        return {
            "point_completeness": len([i for i in issues if i.point is not None])
            / total,
    ...
    def _calculate_quality_score(
        self, completeness: Dict, consistency: Dict, anomalies: Dict
    ) -> float:
        """品質スコア計算 (0-1)"""
        try:
            # 完整性スコア (重要フィールドの平均)
            ...
            completeness_score = sum(
                completeness.get(field, 0) for field in important_fields
            ) / len(important_fields)

            # 一貫性スコア
            consistency_score = (
                1.0 if consistency.get("inconsistency_count", 0) == 0 else 0.8
            )
            ...
            # 総合スコア
            return (
                completeness_score * 0.5 + consistency_score * 0.3 + anomaly_score * 0.2
            )
