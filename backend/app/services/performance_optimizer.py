...
class PerformanceOptimizer:
    ...
    async def optimize_issue_loading(
    ...
    def optimize_chart_calculation(
        self, issues: List[IssueModel], chart_type: str = "burndown"
    ) -> Dict[str, Any]:
        """チャート計算最適化"""
        cache_key = (
            f"{chart_type}_{len(issues)}_{hash(str(sorted([i.id for i in issues])))}"
        )
    ...
    def _calculate_burndown_optimized(self, issues: List[IssueModel]) -> Dict[str, Any]:
        ...
        sorted_issues = sorted(
            [i for i in issues if i.closed_at], key=lambda x: x.closed_at
        )
    ...
    def _calculate_velocity_optimized(self, issues: List[IssueModel]) -> Dict[str, Any]:
        ...
        for issue in completed_issues:
            # 週の開始日を計算
            week_start = issue.closed_at.date() - timedelta(
                days=issue.closed_at.weekday()
            )

            if week_start not in velocity_by_week:
                velocity_by_week[week_start] = {"points": 0, "issues": 0}
