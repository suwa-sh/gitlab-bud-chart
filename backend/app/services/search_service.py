    ...
    def _text_search(self, issues: List[IssueModel], query: str) -> List[IssueModel]:
        ...
        return [
            ...
            if (
                ...
                or (issue.description and query_lower in issue.description.lower())
            )
        ]

    def _apply_filters(
        self, issues: List[IssueModel], filters: Dict[str, Any]
    ) -> List[IssueModel]:
        """フィルタ適用"""
        ...
        if filters.get("created_after"):
            ...
            issues = [
                ...
                for i in issues
                if (
                    i.created_at.astimezone(timezone.utc).date()
                    if i.created_at.tzinfo
                    else i.created_at.date()
                )
                >= created_after
            ]
        ...
        if filters.get("completed_after"):
            ...
            issues = [
                ...
                if i.completed_at
                and (
                    i.completed_at.astimezone(timezone.utc).date()
                    if i.completed_at.tzinfo
                    else i.completed_at.date()
                )
                >= completed_after
            ]
