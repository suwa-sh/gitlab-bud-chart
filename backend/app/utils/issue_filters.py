...
EXCLUDED_KANBAN_STATUSES = ["テンプレート", "ゴール/アナウンス", "不要"]
...
def apply_exclusion_filter(issues: List[IssueModel]) -> List[IssueModel]:
    ...
    return [
        issue for issue in issues if issue.kanban_status not in EXCLUDED_KANBAN_STATUSES
    ]
    ...
    if issue.completed_at and issue.created_at:
        try:
            ...
            if created_at > completed_at:
                logger.warning(
                    f"Issue {issue.iid}: correcting created_at from {created_at} to {completed_at}"
                )
                if not need_copy:
    ...
    if start_date and issue.created_at:
        try:
            ...
            if created_at < start_datetime:
                logger.warning(
                    f"Issue {issue.iid}: correcting created_at from {created_at} to {start_datetime}"
                )
                if not need_copy:
                    corrected_issue = issue.model_copy()
                    need_copy = True
...
def apply_unified_filters(
    issues: List[IssueModel], start_date=None
) -> List[IssueModel]:
    ...
    corrected_issues = [
        apply_date_correction(issue, start_date) for issue in filtered_issues
    ]
...
def apply_scope_filters(
    issues: List[IssueModel], start_date: date, end_date: date
    ...
    for issue in issues:
        ...
        if (
            not normalized_issue_quarter
            or normalized_issue_quarter not in normalized_target_quarters
        ):
            warnings.append({"issue": issue, "reason": "quarter"})
        else:
    ...
    # 3. 日付補正
    corrected_issues = [
        apply_date_correction(issue, start_date) for issue in exclusion_filtered
    ]
    ...
    for issue in corrected_issues:
        ...
        if issue.created_at:
            ...
            if created_at > end_datetime:
                warnings.append({"issue": issue, "reason": "created-after-period"})
                continue
        ...
        if issue.completed_at:
            ...
            if completed_at > end_datetime:
                warnings.append({"issue": issue, "reason": "post-period"})
                continue
            # 期間前完了チェック
            elif completed_at < start_datetime:
                warnings.append({"issue": issue, "reason": "pre-period"})
                continue
    ...
    for issue in scope_filtered:
        if issue.kanban_status in ["完了", "共有待ち"] and not issue.due_date:
            warnings.append({"issue": issue, "reason": "no-due-date"})
        else:
